...
Na manhã de uma segunda-feira de março, abri o dashboard de billing como de hábito e a conta AWS do mês estava quase 6x acima da baseline histórica. Nenhum alarme técnico tinha disparado. Zero impacto pra usuário final, zero perda de dado, zero degradação em serviços vizinhos. Mas o caixa sangrou por 76 horas seguidas e ninguém percebeu.

A Harmo é a Plataforma Drive-to-Store que processa 10 milhões de pesquisas e 300 mil avaliações públicas por mês. Cada um desses números toca uma família de fluxos serverless. Quando um único fluxo entra em loop improdutivo, ele paga o preço pelo restante.

Postmortem público é prática consolidada na engenharia séria de software, e blameless é o nosso padrão interno. Esse caso sai pra fora porque a topologia da falha é generalizável: quem opera Step Functions, máquinas de estado ou qualquer fluxo com loop pode tropeçar na mesma armadilha. Foi o único evento desse perfil no histórico do ambiente, e a categoria de falha não tem mais como acontecer depois das contramedidas que descrevo abaixo.

# A topologia do fluxo

O fluxo afetado é uma Step Function de coleta paginada de avaliações de concorrentes via API externa, agendada para rodar de madrugada. O desenho é trivial em pseudocódigo:
...
Um Choice state que retorna pra invocação da lambda enquanto `$.collector.isFinished == false`. Em operação normal, a lambda esgota a paginação da API de origem em 3 a 8 invocações por execução. Limpinho, funcionou estável por meses.

# A regressão

Numa quarta-feira, 19h05, mergeei um PR que mudou a lógica de paginação dessa lambda. Passou no review, passou nos testes existentes, passou na esteira. Não violava o contrato de I/O da função, todos os campos do payload continuavam com o shape esperado, tipos corretos, schema válido.
...
Cada execução afetada consumia o teto de 25.000 eventos do histórico da Step Function antes de falhar com `States.Runtime — History execution limit exceeded`. Eram cerca de 8 horas e meia de loop, com a Step Function consumindo invocações Lambda e transições SFN em ritmo industrial até bater no limite hard da AWS.

O padrão recorreu em quatro madrugadas consecutivas, atravessando um final de semana inteiro. No total, 76 horas de recorrência intermitente em que cada ciclo entregava `States.Runtime` no fim, e ninguém olhava.
...
**Camada de observabilidade.** Não havia alarme sobre duração de execução de Step Function, sobre número de transições por execução, nem sobre concorrência da lambda contra baseline rolling. A única linha de defesa era o sinal financeiro, que por design tem latência de consolidação de 24 a 48 horas. O Cost Anomaly Detection da AWS requer múltiplos pontos de observação pra estabelecer baseline antes de disparar. Um único evento anômalo pode não ultrapassar o threshold estatístico nos primeiros dias.
...
Trabalhei com a [Infomach](https://www.infomach.com.br/), nossa parceira AWS, articulada via TD Synnex como distribuidor. O time da Infomach atuou em duas frentes em paralelo: validação técnica do diagnóstico e desenho dos guard rails que descrevo abaixo, e abertura do pleito comercial junto à AWS apresentando o postmortem completo, a janela do incidente, a causa raiz e o plano de correção já executado. A AWS reconheceu o caso e concedeu uma fração relevante do delta como crédito. Parceria excelente em ambas as pontas.
