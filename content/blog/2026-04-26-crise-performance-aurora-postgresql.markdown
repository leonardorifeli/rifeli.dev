...
Ninguém planeja uma crise de performance. Ela chega numa quarta-feira de manhã, quando o dashboard fica vermelho e o Slack começa a apitar. Foi mais ou menos assim que começou uma das crises mais instrutivas que atravessei no Aurora PostgreSQL 13.18 — e o que quero contar aqui não é o "fizemos tudo certo", mas a ordem real em que apaguei o fogo, o que funcionou de primeira e o que eu não tinha mapeado direito antes.

Na Harmo, o core transacional roda em Aurora. A massa de eventos é alta — cada pesquisa respondida, cada e-mail aberto, cada webhook de fornecedor bate no banco. Quando esse fluxo degrada, a plataforma inteira sente.

# O sintoma

O primeiro sinal foi CPU de writer subindo de um patamar de 35–45% pra beirar 90% de forma sustentada. Commit latency acompanhou: o p95 de gravação subiu de dezenas de milissegundos pra mais de um segundo. Connections ativas também: o que costumava ficar em torno de 120 foi pra 380 em 15 minutos.
...
1. Uma tabela grande de quarentena de ingestão fazia `INSERT` em alta frequência, um registro por vez, disparado por um consumer que processava eventos vindos do SES.
2. Uma query de leitura recorrente estava em `Seq Scan` num relacionamento de milhões de linhas — índice existia, mas o planner não usava.
...
A tentação numa crise é atacar tudo ao mesmo tempo. Errado. Cada ação precisa ter rollback possível e janela de verificação. Fiz assim:
...
**3. Índice cirúrgico na tabela de quarentena**
...
```sql
CREATE INDEX CONCURRENTLY idx_quarantine_lookup
  ON quarantine_contact (tenant_id, status, created_at DESC);
```
...
- **Índices custam no write path.** Revisar `pg_stat_user_indexes` uma vez por trimestre é barato e evita exatamente esse tipo de surpresa.
- **Single-row inserts são inofensivos em baixa frequência e assassinos em alta.** Qualquer consumer que processa eventos vindo de fila deveria nascer com batch; se foi escrito single-row, é dívida técnica com data de vencimento.
- **Seq scan não é necessariamente o vilão.** Em tabelas pequenas pode ser a escolha certa. Em tabelas grandes, quase nunca — e se o planner está indo por `Seq Scan` mesmo com índice disponível, o índice provavelmente não é o índice certo.
- **Numa crise, ordem importa.** Terminar conexões penduradas primeiro compra tempo. Dropar índices antes de criar novos reduz write amplification antes de adicionar um novo. Mudanças de código ficam pro fim, com deploy calmo.
- **Um alerta bom teria detectado isso antes.** O alarme que tocou reagiu ao sintoma final. Monitorar `pg_stat_user_indexes` e latência de commit teria mostrado a deterioração dias antes. Isso virou item de backlog — e assunto do último post dessa série.

**💬 E você, já passou por crise assim no PostgreSQL?**

Qual foi a ação que mais te salvou? Conta nos comentários, quero comparar runbooks.
